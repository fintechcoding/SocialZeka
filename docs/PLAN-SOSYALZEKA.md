...
| "Hüküm yasağı" (§22.4 ile opt-in'e çevrilmişti) | Opt-in **Kişi kartı: modelin görüşü** paneli `ReadingPrompt`'un sınırlarını kullanır: izlenim dili, simetri, `baska_okuma` zorunlu, dayanaksız madde düşer. "Kişilik/karakter/güçlü-zayıf yan **izlenimi**" bu panelde verilebilir (kullanıcı kararı, §10). "Psikolojik durum / duygu durumu" da **verilir** — 8 Eylül 2026'da gevşetilen kural, §7-4; yerine klinik tanı adı yasaklandı. "Kullanabileceğin argümanlar" istemi yazılmaz — karşılığı kanıt zeminindeki "Elindeki kayıtlar" listesi (§4.4). | `YOLHARITASI.md:39-41` "yorum TAM SERBEST, dürüst paketlemeyle"; `AppSettings.cs:483-485` "bir kişide kusur bulmakla görevli araç gözlemci olmaktan çıkar". |
...
4. **KURAL EVRİMİ — gevşetildi (kullanıcı kararı, 8 Eylül 2026).** Eski hâli: "**Psikolojik durum / duygu durumu / güvenilirlik** analizi (ölçülemez, yanlışı zararlı); kişi kartı bunu yazılı söyler." Kullanıcı kişi kartında psikolojik ve duygusal okuma istedi; yasağın gerekçesi ("Türkçede doğrulanmadı, yanlış etiket yıllarca taşınır") kendisine söylendikten sonra kararını tekrarladı: *"psikolojisini analiz etsin herşeyi analiz edebilir sorun yok benim hakkımdada analiz yapabilir."* Bugün kişi kartı `psikolojik_okuma`, `duygusal_oruntuler` ve `iliskinin_seyri` bölümlerini verir. **Bağlayıcı kalanlar:** her madde dayanağına bağlıdır ve dayanaksızı kodda düşer ve sayılır (`ContactReadingAnalysis.Shape`); tek cümleden duygu çıkarılmaz, örüntü aranır (metin otomatik tanımadan geldiği için yanlış duyulmuş bir kelime birinin hissi olarak kendisine söylenmesin); ses tonu iddiası yoktur. **Yeni yasak: klinik tanı adı** — depresyon, kişilik bozukluğu, sendrom. Bu bir izlenim değil hekim sözüdür ve yanlışı gerçek bir insana yapışır. **Güvenilirlik** ölçüsü yasak kalır, §7-1 ile aynı gerekçeyle: sayı sahte bir kesinlik verir. Aynam hâlâ duygu ölçmez; bu değişiklik yalnız kişi kartıdır. Kaydın kendini kapatma kapısı (`ContactReadingMeasuredNegative`, üst üste üç [Katılmıyorum]) tam da bu yüzden yerinde bırakıldı.
...
10. **KURAL EVRİMİ — daraltıldı (kullanıcı kararı, 8 Eylül 2026).** Eski hâli: "`deception_note` **düzey/değerlendirme**sinin kişi düzeyine kopyalanması ya da herhangi bir isteme beslenmesi; `tactic_evidence`/`deception` satırlarının profil istemine girmesi." Kullanıcı tehdit ve imaların kişi analizinde kullanılmasını istedi. Bugünkü sınır tabloda değil **kaynakta**: çıkarımın kendi baskı işaretleri (`source='pipeline'`) kişi okuması paketine `[B#]` dayanağı olarak girer — `Repository.PressureQuotes`, `ContactReadingAnalysis.MaxTactics = 20`. Giren yalnız beyaz listedeki etiket ve makinenin metinde bulduğu alıntıdır; kullanıcının reddettiği satır girmez. **Değişmeyen yasak:** `deception_note` düzeyi ve değerlendirme paragrafı, ve `source='deception'` taktik satırları hiçbir isteme girmez — o, bir modelin kendi eski şüphesini geri okuyup bir insan hakkında kendi nesrinden dava kurmasıdır. İki satır aynı tabloda durur ve yalnız biri modelin kendi sözüdür; `NothingFromTheAssessmentOrTheSummaryReachesThePrompt` bu ayrımı çiviler.
